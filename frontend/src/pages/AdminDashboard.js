        ...
        return (
          <div>
            ...
            <div className="flex border-b border-blue-200">
              <button
                className={`px-6 py-3 text-lg font-medium ${activeProductsSubTab === 'list' ? 'text-red-500 border-b-2 border-red-500' : 'text-gray-500'}`}
                onClick={() => setActiveProductsSubTab('list')}
              >
                Product List
              </button>
              <button
                className={`px-6 py-3 text-lg font-medium ${activeProductsSubTab === 'add' ? 'text-red-500 border-b-2 border-red-500' : 'text-gray-500'}`}
                onClick={() => setActiveProductsSubTab('add')}
              >
                Add Product
              </button>
              <button
                className={`px-6 py-3 text-lg font-medium ${activeProductsSubTab === 'categories' ? 'text-red-500 border-b-2 border-red-500' : 'text-gray-500'}`}
                onClick={() => setActiveProductsSubTab('categories')}
              >
                Categories
              </button>
              <button
                className={`px-6 py-3 text-lg font-medium ${activeProductsSubTab === 'brands' ? 'text-red-500 border-b-2 border-red-500' : 'text-gray-500'}`}
    ...
    <div className="bg-blue-100 text-gray-800 min-h-screen h-screen flex overflow-hidden">
      ...
      <main className="flex-1 w-full overflow-hidden flex flex-col">
        ...
        <div className="lg:hidden sticky top-0 z-30 bg-white shadow-md p-3 md:p-4 flex items-center border-b border-gray-200">
          ...
          <h1 className="ml-3 md:ml-4 text-lg md:text-xl font-bold text-gray-800">Admin Panel</h1>
        </div>
